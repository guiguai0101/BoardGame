...
describe('Fate/Domination foundation domain', () => {
    ...
    it('相同玩家与种子产生相同的初始实体与区域状态', () => {
        ...
        expect(first.core.players['0'].hand.map((card) => card.id)).toEqual(['0:cardA1', '0:cardA2', '0:cardA3', '0:cardA4', '0:cardB1', '0:cardB2']);
    ...
    it('只允许在部署阶段选择地点，并在地点容量与玩家资源上留下领域结果', () => {
        ...
        expect(FateDominationDomain.validate(state, command('DEPLOY_MASTER', '0', { locationId: 'workshop' })).error).toBe('wrongPhase');
    });
    ...
    it('要求恰好两张攻击牌，并拒绝第三张与过期牌实体', () => {
        ...
        expect(state.core.phase).toBe('battle');
        expect(state.core.battleResult?.power).toBe(7);
    });
    ...
    it('战斗推进后记录代表性 VP 结算，并在第 11 回合后收口', () => {
        ...
        state = apply(state, command('ADVANCE_PHASE', '0', {}));
        state = apply(state, command('DEPLOY_MASTER', '0', { locationId: 'miyama' }));
        state = apply(state, command('SELECT_ATTACK_CARD', '0', { cardId: state.core.players['0'].hand[0].id }));
        state = apply(state, command('SELECT_ATTACK_CARD', '0', { cardId: state.core.players['0'].hand[1].id }));
        state = apply(state, command('CONFIRM_ATTACK', '0', {}));
        state = apply(state, command('ADVANCE_PHASE', '0', {}));
        expect(state.core.players['0'].victoryPoints).toBe(5);
