...
- 用户反馈的是现有拖拽、点击、长按、缩放或其它原交互“难用 / 失效 / 命中困难”时，目标是修复原交互路径：先查坐标、命中区、触控事件、缩放和遮挡，不得未经用户明确要求就新增按钮、HUD、toast、辅助面板或另一套可见入口来替代原问题。
- 关键操作在新设计或验收中被证明只能靠拖拽时，才补 `点击选中 -> 点击目标 -> 点击确认` 的回退路径；这属于新增可见交互，必须先说明影响范围，并按 UI / E2E / 看图规范完成真实入口截图与用户展示。
- 侧栏、日志、帮助和说明可折叠；固定牌桌主构图里的顶栏、分数、手牌、主按钮和中央牌区默认不可另排成移动专版。

### 3.1 壳内坐标与真实视口边界

每个移动适配 UI 在开工前必须先标记为 `in-shell` 或 `portal`：

- `in-shell`：渲染在 `MobileBoardShell` 变换树内，必须使用 PC 设计坐标、`%`、容器尺寸或壳层单位。
- `portal`：通过 `modal-root` / `body` 脱离壳层渲染，才允许使用真实 `vw` / `vh`、安全区和真实视口断点。

`in-shell` 路径禁止直接用以下内容决定尺寸、位置、显隐或承载量：

- `100vw`、`100vh`、`calc(100vw - ...)`、`calc(100vh - ...)`、`vh` / `vw` 主布局单位。
- `sm:`、`md:`、`lg:`、`xl:` 等基于真实浏览器视口的布局断点。
- 因真实视口宽度而 `hidden`、减少槽位、切换成另一套主承载。

如果组件同时服务 `in-shell` 与 `portal`，必须由显式 owner 传入模式或使用独立样式函数；不得让组件内部猜测当前是否被壳层缩放。`portal` 的真实视口单位不能反向证明 `in-shell` 子树正确。
...
3. **锁定壳边界**：列出 `MobileBoardShell` 内的主画布、常驻 HUD、状态栏、动作栏、选择页、阶段浮层、发现 / 事件面板、结算页，以及通过 portal 脱离壳层的阅读 / 放大层。
4. **审查游戏层**：读取 `manifest.ts`、`Board.tsx`、`ui/`；同时排查 `src/index.css`、共享按钮组件和全局媒体查询。
5. **静态扫壳内单位**：对 `in-shell` 文件检索 `vw/vh/100vw/100vh`、真实视口断点和基于视口的 `hidden`；每个命中项必须证明是 portal、迁移回设计单位或保留为明确的移动能力分支。
6. **归类风险**：hover 依赖、drag 依赖、固定尺寸、主体比例、常驻侧栏、信息密度、全局样式污染、命中区和视觉混淆。
7. **选 manifest**：只用 `references/manifest-fields.md` 已批准字段；主验收方向跟 `preferredOrientation` 一致。
8. **按层修**：先框架 / shell，再游戏层轻量例外；未来多游戏复用的能力回推框架层。
9. **验证**：至少覆盖角色 / setup、主牌桌常态、一个主操作后态、一个提示 / 发现 / 事件压力态和结算 / 收口态；每个状态都做 PC 对照、移动主方向 E2E、截图核图和交互语义矩阵。
10. **比例断言**：除了 shell 自身，还要断言代表性一级区域的矩形、可见数量和显隐语义按同一 scale 映射；不能只证明元素存在、未溢出或 shell scale 正确。若使用满载 / 压力态证明不遮挡，必须同时补同入口无压力常态，防止把极端拥挤画面冒充正常体验。
...
- 每个 `in-shell` 一级区域都必须有去向和证据：继续显示、等比缩放、容器内滚动或明确规则阶段互斥；不能只因真实视口断点未命中而消失。
- 至少一条静态检查或 E2E 断言必须能阻止壳内重新引入真实 `vw/vh` 主布局和 `sm/md/lg/xl` 显隐分支。
