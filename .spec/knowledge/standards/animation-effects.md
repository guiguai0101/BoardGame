...
连续动效的真实浏览器录制、GIF 时间保真、远程 / 近战独立覆盖和动态 evidence 收口，统一执行项目 [`e2e-animation-evidence`](../../skills/e2e-animation-evidence/SKILL.md)；本文件不重复维护录制 SOP。
...
- 卡牌或规则对象已经承载的固定说明不在外层摘要重复；投骰 / 重掷的操作入口、实体骰面、总点数和实时结果必须由骰盘或结果层独立承载，不能因去重而被外层文案清理掉。
- 凡是面向全桌公开、会影响后续结算的判定结果，结果层必须保持可读并等待所有真人确认；关闭或转移发生在 `确认 y/y` 之后，不能把“本人已确认”误当成“结果已冻结”。物品或能力只提供改写结果的使用时机；普通物品发现不属于判定结果确认。
...
- 对象移动、位移、推斥、传送和远程攻击必须使用各自的动作语法。短距 / 单格普通移动默认只表现为对象落位，不强制可见 FX；长路径普通移动才使用轻量足迹 / 路径 / 位移轨迹。推斥、传送、冲刺、投射等特殊移动必须按规则语义使用自己的特效，不得复用远程攻击、法术投射物或伤害飞弹的长距离 projectile 语法。
- 召唤、攻击、投射、对象移动、翻页、拖拽等连续动效更新必须生成 GIF 作为最终动态证据；GIF 必须覆盖来源唤醒、过程 / 路径、命中或结果、稳定收口四段（同格效果可没有路径段，但仍要覆盖来源、结果和收口）。关键帧截图只能用于 AI 自检、逐帧定位和辅助索引，没有 GIF 时不得把动效更新判为最终 PASS。
